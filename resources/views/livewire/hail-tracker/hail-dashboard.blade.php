<div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-[300px_1fr]">
        ...
        <div wire:key="hail-map-{{ $selectedDate }}-{{ $filterMinSize }}-{{ (int)$showRadar }}-{{ (int)$showWarnings }}-{{ (int)$showMesh }}"
             ...
             data-mesh-cells="{{ $showMesh ? json_encode($this->meshCells) : '[]' }}"
             data-mesh-cell-count="{{ $showMesh ? count($this->meshCells) : 0 }}"
            ...
            <p class="mt-2 text-xs text-slate-400">
                ...
                @if($showMesh && $this->meshSwathUrl)
                    ...
                    <span class="text-slate-300">(peak {{ number_format($this->meshRecord->max_size_inches, 2) }}"&nbsp;· daily max)</span>
                @elseif($showMesh && !$this->meshSwathUrl)
                    &middot; MESH Swath: <span class="text-slate-300">no data for this date</span>
                @endif
            </p>
        </div>
    </div>
