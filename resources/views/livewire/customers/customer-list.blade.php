<div>
    ...
    @if($this->customers->count())
        <div class="space-y-2">
            @foreach($this->customers as $customer)
                ...
                >
                    <div class="min-w-0">
                        <p class="font-semibold text-slate-900">{{ $customer->full_name }}</p>
                        <div class="mt-0.5 flex flex-wrap gap-x-3 text-xs text-slate-500">
                            @if($customer->phone)
                                <span>{{ $customer->display_phone }}</span>
                            @endif
                            @if($customer->email)
                                <span class="hidden sm:inline">{{ $customer->email }}</span>
                            @endif
                        </div>
                    </div>
                    <div class="ml-3 flex shrink-0 items-center gap-3">
                        <span class="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">
                            {{ $customer->vehicles_count }} {{ Str::plural('vehicle', $customer->vehicles_count) }}
                        </span>
    ...
    @elseif($this->search)
        <div class="rounded-xl bg-white px-6 py-12 text-center shadow-sm ring-1 ring-slate-200">
            <p class="text-sm text-slate-500">No customers found for "<strong>{{ $this->search }}</strong>"</p>
        </div>
    @else
        <div class="rounded-xl bg-white px-6 py-12 text-center shadow-sm ring-1 ring-slate-200">
            ...
            <p class="mt-3 text-sm font-medium text-slate-900">No customers yet</p>
            <p class="mt-1 text-xs text-slate-500">Add your first customer to get started.</p>
            <a href="{{ route('customers.create') }}" class="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">
                Add Customer
            </a>
        </div>
    @endif
